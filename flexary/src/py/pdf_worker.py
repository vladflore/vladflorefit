"""
PDF generation for Pyodide Web Worker context.

No DOM / JS-bridge access — all inputs arrive as plain Python values (JSON
strings / bytes / primitives) via the worker's postMessage channel.  The
function returns raw bytes; the main thread is responsible for creating the
browser download link.

This module is a faithful port of the create_pdf() logic in pdf.py with the
following minimal differences:
  • Replaces `localStorage.getItem(...)` / `catalog.get_exercise(...)` /
    `state.is_authenticated()` with explicit parameters.
  • Removes all `from js import …` DOM imports (unavailable in a worker).
  • Uses pyfetch with absolute URLs (APP_ROOT prefix) for asset fetching.
  • Returns `bytes` instead of triggering a browser download.
"""
...
# ---------------------------------------------------------------------------
# App-root URL — set once by set_app_root() before any generation.
# ---------------------------------------------------------------------------

_APP_ROOT: str = ""

_PDF_ASSET_DIR = Path("/tmp/.pdf-assets")
_PDF_FONT_SOURCES: dict[str, str] = {
...
_PDF_IMAGE_SOURCES: dict[str, str] = {
    "logo-nobg.webp": "assets/logo-nobg.webp",
}

_assets_fetched: bool = False


def set_app_root(url: str) -> None:
    """Called once from the JS worker init snippet with the app's base URL."""
    global _APP_ROOT
    _APP_ROOT = url.rstrip("/") + "/"


# ---------------------------------------------------------------------------
# Asset fetching (fonts + logo)
# ---------------------------------------------------------------------------

async def _ensure_assets() -> None:
    global _assets_fetched
    if _assets_fetched:
        return
    _PDF_ASSET_DIR.mkdir(exist_ok=True)
    for filename, rel_path in {**_PDF_FONT_SOURCES, **_PDF_IMAGE_SOURCES}.items():
        target = _PDF_ASSET_DIR / filename
        if target.exists():
            continue
        url = _APP_ROOT + rel_path
        response = await pyfetch(url)
        data = await response.bytes()
        target.write_bytes(data)
    _assets_fetched = True


# ---------------------------------------------------------------------------
# Helpers (mirrors of the equivalents in models.py / pdf.py)
# ---------------------------------------------------------------------------
...
def _make_t(locale_dict: dict):
    """Return a translation function backed by the supplied locale dict."""
    def t(key: str, **kwargs) -> str:
        msg = locale_dict.get(key, key)
        if kwargs:
            try:
...
async def generate_pdf_bytes(
    *,
    ...
    custom_site_url: str | None = None,
    custom_border_color: str | None = None,
) -> bytes:
    """
    Generate a workout PDF and return it as raw bytes.

    All inputs are plain Python values — no JS proxies.
    """
    ...
    locale_dict: dict = json.loads(locale_json) if locale_json else {}
    t = _make_t(locale_dict)
    catalog_dict: dict = json.loads(catalog_json) if catalog_json else {}
    workouts = workouts_from_json(workouts_json)
    ...
    def _draw_logo_block(pdf_obj, logo_x, logo_y, logo_bytes):
        if logo_bytes is not None:
            try:
                pdf_obj.image(
                    io.BytesIO(logo_bytes), x=logo_x, y=logo_y,
                    w=_logo_size, h=_logo_size,
                )
            except Exception:
                pdf_obj.image(
                    str(_PDF_ASSET_DIR / "logo-nobg.webp"),
                    x=logo_x, y=logo_y, w=_logo_size, h=_logo_size,
                )
        else:
            pdf_obj.image(
                str(_PDF_ASSET_DIR / "logo-nobg.webp"),
    ...
    class PDF(FPDF_cls):
        ...
        def footer(self):
            ...
            page_label = (
                f"{getattr(self, 'workout_page_num', 1)} / "
                f"{getattr(self, 'workout_total_pages', 1)}"
            )
    ...
    for workout in workouts:
        ...
        for i in range(0, len(exercises), chunk_size):
            ...
            for row_num, exercise in enumerate(chunk):
                ...
                is_superset_start = exercise.superset_id and not inside_superset
                follows_superset = (
                    prev_ex is not None
                ...
                if is_superset_start:
                    ...
                    if rest_secs:
                        ...
                        _fmt = (f"{_m}m {_s}s" if _s else f"{_m}m") if _m else f"{_s}s"
                        current_ss_header_text = (
                            f"Superset — {rounds_label}  ·  rest for {_fmt} between rounds"
                        )
                    else:
                ...
                is_custom_ex = int(exercise.id) < 0
                ex_data = catalog_dict.get(str(exercise.id), {})
                if is_custom_ex:
                    _yt_id = (
                        exercise.custom_video_id if is_authenticated else ""
                    ) or (ex_data.get("yt_video_id", "") if ex_data else "")
                    detailed_page_link = (
                        f"https://www.youtube.com/watch?v={_yt_id}" if _yt_id else ""
                    )
                else:
                    if ex_data:
                        detailed_page_link = (
                            f"https://vladflore.fit/flexary/detail.html"
                            f"?exercise_id={exercise.id}"
                        )
                        if is_authenticated and exercise.custom_video_id:
                ...
                try:
                    ...
                    sets = 1
                categories = (
                    [c.strip() for c in ex_data["category"].split(",")]
                    if ex_data
                    else []
                )
                ...
                if detailed_page_link:
                    _name_text_w = (
                        exercise_name_column_width - (_qr_size + 2) - 3 - _tri_size * 1.6 - 2 - 2
                    )
                else:
                ...
                if exercise.notes:
                    ...
                    _text_w = (
                        exercise_name_column_width - _qr_reserved - 3 - _tri_size * 1.6 - 2 - 2
                    )
                    pdf.set_font("opensans", style="", size=7)
                    _lines, _line_w = 1, 0
                ...
                if has_rest:
                    ...
                    _fmt_r = (
                        (f"{_m_r}m {_s_r}s" if _s_r else f"{_m_r}m") if _m_r else f"{_s_r}s"
                    )
                ...
                for cat in categories:
                    ...
                    pdf.rect(
                        badge_x, badge_y, badge_text_w, badge_h,
                        style="F", round_corners=True, corner_radius=1.5,
                    )
                    pdf.set_text_color(255, 255, 255)
                    text_y_b = badge_y + (badge_h - pdf.font_size) / 2
                    pdf.set_xy(badge_x, text_y_b)
                ...
                if detailed_page_link:
                    ...
                    qr_y_pos = row_y + (total_h - qr_size) / 2
                ...
                pdf.set_font("opensans", style="B", size=10)
                pdf.multi_cell(
                    text_w, name_line_h, exercise.name, border=0, align="L",
                ...
                if detailed_page_link and qr_buf is not None:
                    pdf.image(
                        qr_buf, x=qr_x, y=qr_y_pos, w=qr_size, h=qr_size,
                        link=detailed_page_link,
                    )
                ...
                pdf.cell(
                    reps_time_column_width, row_height, reps_time_cell_content, border=0, align="C"
                )
            ...
            if is_last_chunk:
                ...
                pdf.set_x(x_start)
                pdf.rect(
                    x_start, pdf.get_y(), table_width / 2 - 2, executed_h,
                ...
                pdf.rect(
                    notes_x, executed_y, table_width / 2 - 2, notes_h_box,
                    round_corners=True, corner_radius=3,
                )
                ...
                pdf.set_text_color(0, 0, 0)
                pdf.set_y(executed_y + notes_h_box + 4)
